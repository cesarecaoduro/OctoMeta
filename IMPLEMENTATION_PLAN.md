...
- **Acceptance:** Playwright: two sheets in one doc; sheet A publishes `beam.span`, sheet B consumes it; block move never changes values (SCHEMA.md §5); reload restores both snapshots and all graph state.
